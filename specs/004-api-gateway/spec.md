...
- **儲存機制**: 使用 Redis 集中管理計數器,支援多實例部署
  - Redis Key 格式: `ratelimit:{ip}:{minute}` (minute 為 Unix timestamp / 60)
...
### FR-014: 重試機制
- 對後端服務的**寫入操作** (POST/PUT/DELETE) 不自動重試 (避免重複操作，如出價)
- **讀取操作** (GET) 可使用 Polly 重試策略:
  - 最多重試 3 次
  - 使用指數退避 (exponential backoff)
  - 僅在網路錯誤或 5xx 錯誤時重試
- 聚合請求的並行呼叫僅對 GET 請求啟用重試
- 健康檢查失敗時標註服務為 unhealthy，不影響實際請求路由
