...
**Goal**: 任何用戶（含未登入訪客）可瀏覽 Active 商品、依分類篩選、關鍵字搜尋（title ILIKE，大小寫不敏感）、分頁（pageSize=20）。  
...
- [ ] T016 [P] [US1] 撰寫 `tests/AuctionService.UnitTests/Auction/Application/GetAuctionsQueryHandlerTests.cs`：測試無條件查詢回傳分頁結果、關鍵字篩選（ILIKE 大小寫不敏感）、分類篩選、結果為空時 HTTP 200 + empty array、Ended 商品不出現在結果中（共 5 個測試用例）
...
- [ ] T035 [US3] 新增 `src/Modules/Auction/Application/Queries/GetWatchlist/GetWatchlistQuery.cs` 與 `GetWatchlistQueryHandler.cs`：Join Watchlist + `Auction`；`status` 參數為 null 則全部，`"active"` 則篩選 Active；按 AddedAt 降序
...
- [ ] T040 [US4] 新增 `src/Modules/Auction/Infrastructure/BackgroundServices/AuctionEndBackgroundService.cs`：`BackgroundService` 實作；`PeriodicTimer(TimeSpan.FromSeconds(60))`；每輪開始時使用 `using var scope = _logger.BeginScope(new { TraceId = Activity.Current?.Id ?? Guid.NewGuid().ToString() })` 結構化日誌追蹤；查詢 `status=Active AND end_time <= now LIMIT 100`；對每筆呼叫 `IBiddingQueryService.GetHighestBidAsync()` 取得 `BidInfoDto?`；呼叫 `auction.End(bidInfo?.WinnerId, bidInfo?.Amount)`；`SaveChangesAsync()`；**若 `bidInfo != null`（有出價紀錄）**則透過 `IMediator.Publish(new AuctionWonEvent(...))`；try-catch 外層 catch Exception → `_logger.LogError(ex, "...")` → continue
...
- [ ] T042 [P] [US5] 撰寫 `tests/AuctionService.UnitTests/Auction/Application/CreateAuctionCommandHandlerTests.cs`：測試成功建立（回傳新 Guid）、起標價 ≤ 0 拋 ValidationException、startTime ≤ now 拋 ValidationException、endTime 未 > startTime+1min 拋 ValidationException（共 4 個測試用例）
...
## Phase 8: User Story 6 — 編輯拍賣商品（賣家） (Priority: P6)

**Goal**: 賣家 `PUT /api/auctions/{id}` 依競標開放狀態分兩階段修改商品：**競標開放前**（`now < startTime`）可修改全欄位（含 startingPrice/startTime/endTime）；**競標開放後**（`now >= startTime`）僅允許 title/description/categoryId/imageUrls，帶入競標敏感欄位則 422；Ended 不可改（409）；非擁有者（403）。  
**Requires JWT**.

**Independent Test**: 建立 `startTime` 在未來的 Active 商品：（1）`startTime` 前提交含 startingPrice 的更新得 200；（2）模擬競標已開放，再次嘗試更新 startingPrice 得 422；（3）Ended 商品更新得 409；（4）其他用戶操作得 403。
...
- [ ] T051 [US6] 新增 `src/Modules/Auction/Application/Commands/UpdateAuction/UpdateAuctionCommandValidator.cs`：`Title` 若 not null 則 NotEmpty MaxLength(200)；`ImageUrls` MaximumCount(5) 且每個 URL 必須為有效絕對 URI；`StartingPrice` 若提供則 GreaterThan(0)；`StartTime` 若提供則 GreaterThan（UtcNow）；`EndTime` 若提供則需 > StartTime+1min（需存取現有商品 StartTime 來計算）；**競標敏感欄位（StartingPrice/StartTime/EndTime）的驗證在 Handler 內依 startTime 状態執行，Validator 只做格式驗證**；`CategoryId` 本 Phase 不驗證存在性
- [ ] T052 [US6] 新增 `src/Modules/Auction/Application/Commands/UpdateAuction/UpdateAuctionCommandHandler.cs`：查找 `Auction`；不存在拋 `NotFoundException`；Status==Ended 拋 `ConflictException`；OwnerId ≠ RequesterId 拋 `ForbiddenException`；檢查 `DateTimeOffset.UtcNow < auction.StartTime`（競標開放前）→ 呼叫 `auction.UpdateAll(...)`；否則（競標已開放）若請求含 startingPrice/startTime/endTime 則拋 `ValidationException`，否則呼叫 `auction.UpdateNonSensitive(...)`；`SaveChangesAsync()`；回傳更新後 `AuctionDetailDto`
...
- [ ] T060a [P] 效能基準測試（Constitution IV 要求）：**方法：在整合測試 `PerformanceTests.cs` 中使用 `Stopwatch` 計時斷言**—事先透過 Seed Helper 植入 10,000 筆 Active 商品，執行 `GET /api/auctions` 連續 10 次取 p95 執行時間驗證 ≤ 500ms；其他端點（Detail/Create/Update/Watchlist）驗證 p95 ≤ 200ms；**此測試僅在本地執行，不列入 CI pipeline**（機器差異導致不穩定）
...
Phase 1 (Setup)
    ...
    │       └── T015a (Seed) ← T017 分類篩選測試依賴
            ├── Phase 3 (US1: Get Auctions)
            ├── Phase 4 (US2: Get Detail)     ← T023a (AuctionImageDto) → T024
            ├── Phase 5 (US3: Watchlist)
            ├── Phase 6 (US4: Background Service)
            ├── Phase 7 (US5: Create Auction)
            └── Phase 8 (US6: Update Auction) ← T049a (UpdateAuctionRequest) → T053
Phase 9 (Scaffold 測試更新) ← 滞與 Phase 1 同步進行
Phase 10 (Polish) ← 依賴所有 User Story Phase 完成
    └── T060a (效能測試) ← 需要 10,000 筆 Active 商品，建議在 T059 Seed 驗證後執行
```
