...
實作 Auction 模組的完整拍賣商品管理功能：賣家建立拍賣商品（建立即 Active，無草稿步驟）、依競標開放狀態分兩階段編輯商品（`startTime` 前可修改全欄位包含 startingPrice/startTime/endTime；`startTime` 後僅限 title/description/categoryId/imageUrls）、瀏覽商品列表（關鍵字搜尋僅比對 title 且大小寫不敏感、分類篩選、分頁 20 筆）、商品詳情（含目前最高出價，由 `IBiddingQueryService` null stub 提供）、競標者加入/移除追蹤清單（冪等）、查詢追蹤清單（預設含 Active+Ended，`?status=active` 篩選）、`AuctionEndBackgroundService` 排程結標（每 60 秒掃描，批次上限 100 筆，發布 `AuctionWonEvent`）。狀態機：`Active → Ended`（無 Draft，無逆轉）。
...
> **注意**：plan.md 的 Phase 編號與 tasks.md 的 Phase 編號不同，以下為對照表：

| plan.md Phase | tasks.md Phase | 內容說明 |
|---------------|---------------|--------|
| Phase 1 (Setup) | Phase 1 (Setup) | 建立 Domain 實體、Events、Abstractions |
| Phase 2+3 (Domain+Abstractions) | Phase 2 (Foundational) | EF Core、Migration、DI |
| Phase 4–8 (US1–US5) | Phase 3–7 (US1–US5) | 拍賣者 + 排程結標 + 建立商品 |
| Phase 9 (US6) | Phase 8 (US6) | 編輯商品（StartTime 分兩階段） |
| Phase 10 (Migration) | Phase 2 T015 | 已入 Foundational Phase |
| Phase 11 (Polish) | Phase 9+10 | Scaffold 測試更新 + Polish |

> **tasks.md 為權威來源**：實作時以 tasks.md 的 Phase 編號為準。
...
| 8 | US5 CreateAuction | 建立商品（直接 Active，Validator 含 startTime） |
| 9 | US6 UpdateAuction | 編輯商品（StartTime 分兩階段，403/409/422） |
| 10 | EF Migration | auction schema migration（删除舊 migration 重建，含 start_time index） |
| 11 | Polish | Integration tests、覆蓋率門法、Logging、XML 文件 |
