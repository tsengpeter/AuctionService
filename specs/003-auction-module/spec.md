...
5. **Given** 追蹤清單中有商品，**When** 用戶查詢追蹤清單，**Then** 回傳含商品基本資訊的清單（含 Active 與 Ended 商品，包含 status 欄位）。
6. **Given** 追蹤清單中有 Active 與 Ended 商品，**When** 用戶帶 `?status=active` 篩選查詢，**Then** 只回傳 Active 商品。
7. **Given** 未登入用戶，**When** 嘗試加入追蹤清單，**Then** 系統回傳未授權（401）。
...
- 商品列表只顯示 Active 狀態的商品（Ended 不出現在公開列表）。
