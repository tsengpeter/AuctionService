# Member Service Development Context
...
- **Language/Framework**: ASP.NET Core 9, C# 12
...
- **Database**: 
  ...
  - **建置方式**: EF Core 9 Code-First Migrations (資料庫結構由程式碼自動產生)
- **Primary Dependencies**:
  - `IdGen` 3.x - Snowflake ID 產生器（64-bit 分散式唯一識別碼）
  - `BCrypt.Net-Next` 4.0.3 - 密碼雜湊（bcrypt + snowflakeId，work factor 12）
  - `System.IdentityModel.Tokens.Jwt` 7.0.3 - JWT 驗證（HS256 對稱式加密）
