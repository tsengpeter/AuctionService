...
- Q: 驗證碼設定? → A: 6位數字、有效期10分鐘、最多錯誤3次、發送冷卻時間60秒
- Q: 通知服務整合? → A: 優先使用雲端服務(AWS SES + AWS SNS)，次之 SMTP + Twilio
- Q: 密碼重設後的安全措施? → A: 撤銷所有現有的 Refresh Token，強制所有裝置重新登入
- Q: 驗證功能的設計? → A: 採用通用驗證機制，使用 VerificationToken 實體支援多種驗證類型（註冊、更新信箱、更新手機等），便於程式碼重用和未來擴展
...
6. **Given** 驗證碼已發送, **When** 超過 10 分鐘未使用, **Then** 系統使該驗證碼失效
...
4. **Given** 驗證碼已過期（超過 10 分鐘）, **When** 使用者嘗試使用過期驗證碼, **Then** 系統拒絕並提示「驗證碼已過期，請重新獲取」
...
- **FR-002-3**: 系統必須生成 6 位數驗證碼，有效期限 10 分鐘
...
- **FR-002-8**: 系統必須允許使用者在驗證碼有效期內（10分鐘）但超過冷卻時間（60秒）後重新提交註冊，此時發送新的驗證碼並將舊驗證碼標記為失效
...
- **密碼重設權杖 (PasswordResetToken)**: 代表密碼重設的臨時驗證碼，包含使用者ID、6位數驗證碼、發送方式（Email/SMS）、過期時間（10分鐘）、使用狀態、錯誤嘗試次數
