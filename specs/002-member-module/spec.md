...
- **FR-013b**: 系統 MUST 對登入端點套用 `"login-ip"` 固定視窗速率限制（每分鐘每 IP 最多 10 次請求，無論成功或失敗），防止大量請求佔用伺服器資源；此機制獨立於 FR-013 失敗次數計數，兩層限制同時生效
...
- Q: 登入失敗多次時，系統是否需要速率限制或帳號鎖定機制？ → A: 實施 IP 速率限制（同 IP 每分鐘累積失敗次數超過 5 次（第 6 次起）回傳 429），不鎖帳號，以避免被攻擊者觸發合法使用者帳號的 DoS
...
- 登入端點實施 IP 速率限制（同 IP 每分鐘累積失敗次數超過 5 次（第 6 次起）回傳 429），不採用帳號鎖定策略以防止 DoS
