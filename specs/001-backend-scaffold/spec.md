...
- **FR-004**: 所有 API 回應 MUST 遵循統一的結構，包含：操作結果（成功/失敗）、資料內容（成功時）、錯誤訊息（失敗時）、HTTP 狀態碼
- **FR-005**: 系統 MUST 支援 JWT 身份驗證，所有需要登入的端點在未提供有效 token 時回傳 401
...
- **API Response（統一回應）**: 所有 API 操作的回應容器，結構為 `{ success, data?, error?, statusCode }`，無論哪個模組皆相同
...
- 模組間非同步通訊透過 In-Memory 事件匯流排，不使用外部Message Broker（此為初期設計，後續可升級）
