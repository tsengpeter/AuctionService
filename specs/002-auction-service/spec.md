...
### 使用者故事 5 - 系統負載測試與效能驗證 (優先順序: P3)

系統在高並發負載下仍能維持穩定性與效能，確保商品查詢、建立、追蹤等核心功能在壓力情境下正常運作。

**優先順序理由**: 效能測試是確保系統可擴展性的關鍵，但可在核心功能完成後進行。不影響基本功能開發，屬於品質保證階段。

**獨立測試**: 可透過負載測試工具（NBomber, JMeter）模擬大量並發請求，監控回應時間、吞吐量、資源使用率等指標，驗證系統在壓力下的表現。

**驗收情境**:

1. **Given** 系統部署完成, **When** 100 並發使用者同時查詢商品列表, **Then** P95 回應時間 ≤ 200ms, 成功率 ≥ 99.5%
2. **Given** 系統運行中, **When** 500 並發使用者高頻輪詢即時出價 API (每 2 秒), **Then** P95 回應時間 ≤ 300ms, Circuit Breaker 正常運作
3. **Given** 熱門商品存在, **When** 1000 並發使用者同時查詢同一商品, **Then** P95 回應時間 ≤ 250ms, 無 N+1 查詢問題
4. **Given** 系統運行中, **When** 300 並發使用者執行混合操作（90% 讀 + 10% 寫）持續 5 分鐘, **Then** P95 回應時間 ≤ 300ms, 資源使用穩定
5. **Given** BiddingService 不可用, **When** 使用者查詢即時出價, **Then** 系統返回降級回應（StartingPrice），P95 回應時間 ≤ 500ms
6. **Given** 系統運行中, **When** 50 並發使用者執行商品建立操作, **Then** P95 回應時間 ≤ 400ms, 成功率 ≥ 99.5%
7. **Given** 系統運行中, **When** 執行大分頁查詢（pageNumber=10000）, **Then** 回應時間 ≤ 1000ms 或返回 400, 無全表掃描
8. **Given** 150 並發使用者超過資料庫連線池上限, **When** 執行複雜查詢, **Then** 連線池滿時返回 503, 連線正常釋放
9. **Given** 系統運行中, **When** 100 並發使用者同時追蹤/取消追蹤商品, **Then** P95 回應時間 ≤ 300ms, 無重複追蹤記錄
10. **Given** 系統運行中, **When** 200 並發使用者執行需認證操作（不同 JWT）, **Then** 認證開銷 ≤ 10ms, P95 回應時間 ≤ 250ms
...
- 當系統承受 500+ 並發使用者壓力時，回應時間與資源使用如何？
- 當資料庫連線池耗盡時，系統如何處理與恢復？
- 當執行極端分頁查詢（page=10000）時，系統如何防護？
...
- **FR-032**: 系統必須通過 P0 關鍵路徑負載測試（商品列表、商品詳情、即時出價查詢）
- **FR-033**: 系統必須在 BiddingService 不可用時提供降級回應（返回 StartingPrice）
- **FR-034**: 系統必須在高並發情境下維持資料一致性（追蹤、建立、更新操作）
- **FR-035**: 系統必須實作資料庫查詢優化（AsNoTracking, 索引使用, 避免 N+1）
- **FR-036**: 系統必須配置適當的連線池大小（MinPoolSize=5, MaxPoolSize=100）
...
- **SC-011**: 在 500 並發使用者壓力下，P95 回應時間維持 ≤ 300ms
- **SC-012**: 熱門商品（1000 並發查詢）回應時間 P95 ≤ 250ms
- **SC-013**: BiddingService 不可用時，降級回應在 500ms 內完成
- **SC-014**: 資料庫連線池耗盡時，系統能在 60 秒內恢復
- **SC-015**: 負載測試成功率達 99.5%（所有情境平均）
